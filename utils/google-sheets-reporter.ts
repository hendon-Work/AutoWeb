...
class GoogleSheetsReporter implements Reporter {
  ...
  onTestEnd(test: TestCase, result: TestResult) {
    ...
    const priorityAnnotation = test.annotations.find(
      (a) => a.type.toLowerCase() === "priority",
    );
    ...
    const getAnnotation = (type: string) =>
      test.annotations.find((a) => a.type.toLowerCase() === type.toLowerCase())
        ?.description || "-";
    ...
    if (errorMsg) {
      this.rows[rowKey]["Error Message"] +=
        (this.rows[rowKey]["Error Message"] ? " | " : "") +
        `[${projectName}] ` +
        errorMsg;
    }
  ...
  async onEnd(result: FullResult) {
    ...
    if (!spreadsheetId || !serviceEmail || !privateKey) {
      console.warn(
        "⚠️ (경고) 구글 스프레드시트 전송용 인증키 정보가 없어 업로드를 건너뜁니다.",
      );
      return;
    }
    ...
    try {
      ...
      ["Total", "Chrome", "Firefox", "Safari"].forEach((key) => {
        ...
        s.p0Rate =
          s.p0Total > 0
            ? Math.round((s.p0Passed / s.p0Total) * 100) + "%"
            : "0%";
      });
      ...
      console.log(
        `✅ 성공적으로 구글 스프레드시트에 실시간 리포트가 새 탭 [${sheetTitle}]에 작성되었습니다!`,
      );
    } catch (error) {
