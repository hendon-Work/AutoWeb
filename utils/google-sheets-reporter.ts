    ...
    async onEnd(result: FullResult) {
        ...
        try {
            ...
            const sheet = await doc.addSheet({
                ...
                headerValues: ['No.', 'Browser', 'Priority', 'File', 'Check List', 'Result', 'Date', 'Duration(ms)', 'Error Message'],
            ...
            await sheet.updateDimensionProperties('COLUMNS', { pixelSize: 80 }, { startIndex: 1, endIndex: 2 }); // 대상 컬럼 인덱스 1 (Browser)
            await sheet.updateDimensionProperties('COLUMNS', { pixelSize: 60 }, { startIndex: 2, endIndex: 3 }); // 대상 컬럼 인덱스 2 (Priority)
            await sheet.updateDimensionProperties('COLUMNS', { pixelSize: 129 }, { startIndex: 3, endIndex: 4 }); // 대상 컬럼 인덱스 3 (File)
            await sheet.updateDimensionProperties('COLUMNS', { pixelSize: 600 }, { startIndex: 4, endIndex: 5 }); // 대상 컬럼 인덱스 4 (Check List)
            await sheet.updateDimensionProperties('COLUMNS', { pixelSize: 164 }, { startIndex: 6, endIndex: 7 }); // 대상 컬럼 인덱스 6 (Date)
            ...
            await sheet.loadCells(`A1:N${this.rows.length + 10}`); // 열 1개 추가로 N열까지 범위 확장

            // 사용자 요청 양식(왼쪽 블록: Row 3, 4 / Col G, H)
            const leftData = [
                ['Total', total],
                ['P0 Total', p0Total]
            ];
            ...
            for (let r = 0; r < 7; r++) {
                ...
                // 데이터 열 (Col J = index 9 ~ M = 12) - 요청 이미지와 동일하게 4칸 구성
                for (let c = 9; c <= 12; c++) {
            ...
            // 본문 영역 스타일 지정 (Browser, Priority, File 정렬, Result 컬러 및 정렬)
            for (let i = 0; i < this.rows.length; i++) {
                const rowIndex = i + 10; // 실제 데이터는 11행(인덱스 10)부터 시작

                // Browser 열(인덱스 1) 가운데 정렬
                const browserCell = sheet.getCell(rowIndex, 1);
                browserCell.horizontalAlignment = 'CENTER';

                // Priority 열(인덱스 2) 가운데 정렬
                const priorityCell = sheet.getCell(rowIndex, 2);
                priorityCell.horizontalAlignment = 'CENTER';

                // File 열(인덱스 3) 가운데 정렬
                const fileCell = sheet.getCell(rowIndex, 3);
                fileCell.horizontalAlignment = 'CENTER';

                // Result 열(인덱스 5) 배경 컬러 및 가운데 정렬 부여
                const statusCell = sheet.getCell(rowIndex, 5);
