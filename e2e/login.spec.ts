...
test.describe('로그인 페이지 유효성 검사 테스트', () => {
    ...
    test('정상적인 계정으로 로그인 성공 테스트', { 
        tag: '@P0',
        annotation: [
            { type: '2depth', description: '로그인' },
            { type: '3depth', description: '로그인 성공' },
            { type: 'precondition', description: '로그인 페이지 접속 완료' },
            { type: 'teststep', description: '유효한 아이디/비밀번호 입력 후 로그인 클릭' },
            { type: 'expectedresult', description: '상품 목록(inventory) 페이지로 정상 이동됨' }
        ]
    }, async ({ page }) => {
    ...
    test('아이디를 입력하지 않고 로그인 시도', { 
        tag: '@P1',
        annotation: [
            { type: '2depth', description: '로그인' },
            { type: '3depth', description: '입력 유효성' },
            { type: 'precondition', description: '로그인 페이지 접속 완료' },
            { type: 'teststep', description: '아이디 빈칸 상태에서 비밀번호 입력 후 로그인 클릭' },
            { type: 'expectedresult', description: 'Username is required 에러 메시지가 표시됨' }
        ]
    }, async () => {
    ...
    test('비밀번호를 입력하지 않고 로그인 시도', { 
        tag: '@P1',
        annotation: [
            { type: '2depth', description: '로그인' },
            { type: '3depth', description: '입력 유효성' },
            { type: 'precondition', description: '로그인 페이지 접속 완료' },
            { type: 'teststep', description: '아이디 입력 후 비밀번호 빈칸 상태에서 로그인 클릭' },
            { type: 'expectedresult', description: 'Password is required 에러 메시지가 표시됨' }
        ]
    }, async () => {
    ...
    test('블락(잠긴) 처리된 계정으로 로그인 시도', { 
        tag: '@P2',
        annotation: [
            { type: '2depth', description: '로그인' },
            { type: '3depth', description: '계정 상태' },
            { type: 'precondition', description: '로그인 페이지 접속 완료' },
            { type: 'teststep', description: '잠긴 계정(locked_out_user) 정보 입력 후 로그인 클릭' },
            { type: 'expectedresult', description: 'user has been locked out 에러 메시지가 표시됨' }
        ]
    }, async () => {
